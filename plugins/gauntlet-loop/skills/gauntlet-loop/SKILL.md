...
4. **Critic clauses must be explicit and checkable ("run X, grep Y, curl Z, confirm result"), not a vague comparison ("as good as some reference app").** Write numbered clauses that can be independently re-verified, not a subjective quality bar. **The clause is frozen before the builder's first attempt and the builder cannot redefine, loosen, or reinterpret it** — this generalizes rule 4b's "gate-in" language beyond just the parametrized-test case: whoever decomposes the piece owns the pass bar, not whoever implements it. Every critic prompt should also say: *"do not pass because a self-report claims it works"* — verify by doing, not by reading what the builder said it did. **When the builder wrote its own test file as part of the piece (the common case — a fix and its regression test land together), the critic must also scrutinize the test itself, not just whether it passes**: reject a test that's tautological, hardcodes the expected output to match whatever the implementation returns, only imports the module without asserting real behavior, or never actually exercises the failing path the piece claims to fix. A passing test authored by the same agent that wrote the fix is real evidence only if it would have genuinely failed against the pre-fix code — the critic should be able to say why, not just that the test is green. On a **retry** critic specifically, ask for maximum specificity in any failure: name the exact clause, file/line, and observed-vs-expected — a vague failure on a retry is how diagnostic detail gets lost twice.

   **4c. For a genuinely high-stakes piece (security, financial, anything a gamed test would actually hurt you on), have a separate agent author the test file BEFORE the builder runs, and hand the builder only implementation work against it — not a new mechanism, just a specific way to compose this skill's existing pieces.** Add one `agent()` call before the pieces list is finalized: it writes the test file for that piece and returns its content/path; that becomes part of the piece's frozen clause (see rule 4 above), and the builder's ownership excludes the test file entirely. This is the full version of authorship-independence, not just verification-independence — reserve it for the pieces that actually warrant the extra turn, not as a default (most pieces are one-off fixes at `MAX_ATTEMPTS = 3`, far below the sustained-iteration pressure this level of separation defends against).
